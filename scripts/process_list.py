...
inf = open(fn)
outf = open('gen/'+fn.replace('.list','.html'), 'w')
if fn.startswith('All_'):
    slidef = FakeFile()
else:
    slidef = open('gen/'+fn.replace('.list','-slides.html'), 'w')

title = fn.replace('.list','').replace('_',' ')
...
if isinstance(slidef, io.IOBase):
    outf.write("<a class=rawgit href=%s>Autogenerated Slides</a>" % fn.replace('.list','-slides.html'))
